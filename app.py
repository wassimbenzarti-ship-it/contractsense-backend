...
def analyze_contract(contract_text, lang, contract_type, api_key, partie="la partie bénéficiaire", file_bytes=None, filename="", progress_cb=None, director_email="", user_models_extra=None, interface_lang="fr"):
    ...
    system = (
        ...
        "- RÈGLE CRITIQUE — proposed doit être une CLAUSE JURIDIQUE COMPLÈTE: proposed doit être rédigé en style contractuel formel et professionnel, de longueur comparable à l'original."
        " INTERDIT de rédiger proposed sous forme de résumé synthétique, de liste à puces, de tirets ou de points numérotés."
        " INTERDIT d'utiliser des abréviations informelles (ex: '>50%', '≥2 membres', 'AGO valide si...')."
        " Le proposed doit pouvoir remplacer l'original tel quel dans le contrat signé — il doit donc être complet, formel, et rédigé en prose juridique.\n"
...
def analyze_adverse_diff():
    ...
    try:
        ...
        # Build structured map of our validated positions for real comparison
        our_positions_map = {}
        for m in our_mods[:25]:
            name = m.get('clause_name', '')
            if name:
                our_positions_map[name] = (m.get('proposed') or m.get('original', ''))[:400]

        our_positions_block = "\n".join(
            [f'"{name}": "{text}"' for name, text in our_positions_map.items()]
        ) if our_positions_map else "(aucune clause validée)"

        client = anthropic.Anthropic(api_key=os.environ.get("ANTHROPIC_API_KEY", ""))
        prompt = f"""Tu es expert juridique. Compare le contrat adverse avec nos positions validées et identifie UNIQUEMENT les vraies divergences.

NOS POSITIONS VALIDÉES — pour COMPARAISON UNIQUEMENT, ne jamais copier dans "proposed":
{our_positions_block}

DOCUMENT ADVERSE (extrait):
{adverse_text[:8500]}

MISSION: Pour chaque clause du document adverse, compare-la avec nos positions validées ci-dessus.
Retourne UNIQUEMENT les clauses adverses qui s'écartent significativement de notre position.
Si une clause adverse est alignée avec notre position validée, NE LA RETOURNE PAS.

RÈGLES ABSOLUES pour le champ "proposed":
1. Pars TOUJOURS du texte de la clause adverse (champ "original")
2. Modifie CE texte adverse pour protéger les intérêts de {partie or 'notre client'}
3. INTERDIT de copier nos positions validées mot pour mot dans "proposed"
4. INTERDIT d'inventer un montant ou une référence absente du texte adverse original
5. Rédige dans la MÊME LANGUE que le texte adverse original
6. "proposed" = version retravaillée du texte adverse, pas un texte générique ou copié

Réponds UNIQUEMENT en JSON valide (tableau), sans markdown:
[{{"clause_name":"nom exact de la clause adverse","original":"texte exact tel qu'il apparaît dans le document adverse","proposed":"version modifiée du texte adverse ci-dessus protégeant {partie or 'nos intérêts'}","risk":"high|medium|low","reason":"explication de la divergence avec notre position validée et pourquoi c'est problématique"}}]

Si aucune divergence significative: retourne []"""
...
def compare_adversary():
    ...
    try:
        ...
        for m in our_mods:
            mid = str(m.get("id", ""))
            if decisions.get(mid) == "accepted":
                orig = m.get("original", "")
                prop = m.get("proposed", "")
                if orig and prop and orig in our_final:
                    our_final = our_final.replace(orig, prop, 1)
...
def chat():
    ...
    try:
        ...
        _legal_sourcing_rules = (
            ...
            "2. Si la BASE LÉGALE contient des extraits pertinents, cite-les et appuie ta réponse dessus.\n"
            "3. Si la BASE LÉGALE ne couvre PAS le sujet posé (extraits non pertinents ou absents) :\n"
            "   a) Pour les PRINCIPES GÉNÉRAUX, CADRES LÉGAUX et questions conceptuelles "
            "(existence d'un régime juridique, logique d'une institution, règles générales applicables) : "
            "RÉPONDS depuis ta connaissance générale en commençant par '📚 D'après ma connaissance générale "
            "(non confirmée par ma base documentaire propriétaire) :' puis donne une réponse complète et utile, "
            "et termine par '⚠ Pour toute application pratique ou décision, vérifiez les textes officiels en vigueur.'\n"
            "   b) Pour les DONNÉES CHIFFRÉES PRÉCISES (durées exactes, montants, seuils, numéros d'articles) : "
            "donne le principe général depuis ta connaissance en précisant que le chiffre exact doit être vérifié — "
            "INTERDIT de citer un chiffre précis sans source RAG car les textes évoluent.\n"
            "4. INTERDIT ABSOLU : citer un chiffre précis (ex. '6 ans', '60 jours', 'art. 264') comme certain "
            "SANS l'avoir dans la BASE LÉGALE. Donne le principe, signale que le chiffre doit être vérifié.\n"
            "5. INTERDIT : inventer une liste de lois 'disponibles' dans la base, ou prétendre ne pas pouvoir "
            "répondre à une question générale de droit quand tu disposes d'une connaissance de formation sur ce sujet.\n"
        )
        ...
        if general_qa:
            ...
            system_prompt = (
                ...
                "Réponds toujours dans la langue de l'utilisateur, de manière professionnelle, claire et synthétique.\n"
                ...
                "Quand la BASE LÉGALE ne couvre pas le sujet, utilise ta connaissance générale "
                "avec le disclaimer approprié (voir règles ci-dessous) — ne refuse JAMAIS de répondre "
                "à une question juridique générale que tu es en mesure de traiter.\n"
