...
def chat():
    ...
    try:
        ...
        system_prompt = (
            ...
            f"3. INSTRUCTION CRITIQUE — BLOC MODIFICATION SYSTÉMATIQUE: Dès que ta réponse porte "
            f"sur une clause ou un article spécifique du contrat, tu DOIS TOUJOURS et SYSTÉMATIQUEMENT "
            f"proposer une rédaction améliorée plus protectrice pour {partie}, même si l'utilisateur "
            f"ne l'a pas demandé. Termine OBLIGATOIREMENT ta réponse par ce bloc EXACTEMENT "
            f"(sur une seule ligne, guillemets doubles uniquement):\n"
            f"<modification>{{\"clause_name\":\"Nom\",\"original\":\"Texte actuel extrait du contrat\",\"proposed\":\"Nouvelle rédaction complète et protectrice\",\"risk\":\"medium\",\"reason\":\"Explication de l'amélioration\"}}</modification>\n"
            f"Règles strictes: JSON sur UNE SEULE LIGNE, guillemets doubles, pas de backticks ni de blocs code. "
            f"TOUJOURS inclure ce bloc si une clause est mentionnée. Ne pas inclure seulement pour les questions purement générales sans lien avec une clause précise.\n"
