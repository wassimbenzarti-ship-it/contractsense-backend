...
"
        "7. CLAUSES DE RÉSILIATION — PRIORITÉ ABSOLUE: Analyser en premier et avec le plus grand soin toutes les clauses de résiliation/terminaison/rupture. Vérifier: (a) conditions de résiliation unilatérale et asymétries; (b) délais de préavis; (c) indemnités de résiliation (absentes = HIGH); (d) effets post-résiliation et survivance des obligations; (e) causes de résiliation automatique vagues (= HIGH); (f) droits en cas de force majeure ou changement de contrôle. Clause résiliation déséquilibrée = toujours risk=high.
